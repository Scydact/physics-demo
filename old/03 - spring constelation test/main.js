...
class Camera {
    ...
    update(realWidth = null, realHeight = null) {
        ...
        showLog(
            ...
            + '\nx: ' + this.viewboxRect.center.x.toFixed(2)
            + '  y: ' + this.viewboxRect.center.x.toFixed(2)
        );
    }
...
class Player {
    ...
    update(step, app) {
        ...
        var speed = (controls.shift) ? 2 * this.speed : this.speed;
...
class Spring {
    constructor(base, restPos, initialPos, initialV, k = 7, m = 1, b = 0, g = 0) {
        ...
        this.tmp = new Array(5).fill(0).map(() => Vector2.zero());
    ...
    update(dt) {
        let { tmp, x, x_rest, dx, accel, b, k, m, g, pos, base } = this;
        // tmp[1].set(dx).smult(-b); // -bx'
        // tmp[2].set(x).sub(x_rest).smult(-k);  // -kx
        // accel.set(tmp[1].add(tmp[2])).sdiv(m).add(g);
        accel.weightSum(0, dx, -b, x, -k, x_rest, k).sdiv(m).add(g);

        dx.add(tmp[0].set(this.accel).smult(dt));
        x.add(tmp[0].set(this.dx).smult(dt));
        pos.set(tmp[0].set(base).add(x))
...
class App {
    ...
    constructor(mainNode, controlsNode) {
        ...
        this.fpsNode.classList.add('pre-wrap')
        ...
        //     new Vector2(0, 400),
        //     7, 1, 0, 0)
        this.springs = new Array(30000).fill(0).map(() => new Spring(
            new Vector2(rn(20000), rn(20000)),
            new Vector2(ri(20000), ri(20000)),
            new Vector2(rn(5000), rn(5000)),
            new Vector2(rn(500), rn(500)),
            ri(15, 5), ri(3, 0.5), ri(0.1), 1e3));
